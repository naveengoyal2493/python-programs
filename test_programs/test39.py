...
# print(stocks_buy_and_sell([5,5,1,3,10,2]))
...
class Solution:
    ...
    def sort_array(self, arr):
        ...
        while i <= right:
            if arr[i] == 0:
                arr = self.swap(arr, left, i)
                left += 1
            elif arr[i] == 2:
                arr = self.swap(arr, i, right)
                right -= 1
                i -= 1
            i += 1
        return arr

# print(Solution().sort_array([2,2,1,1,0,0]))
...
class NextPermutation:
    ...
    def next_permutation(self, nums):
        ...
        while nums[next_num] <= nums[dec] and next_num < len(nums):
            next_num += 1
        nums = self.swap(nums, next_num, dec)
        return nums


# print(NextPermutation().next_permutation([5,1,2,4]))
...
print(maximum_subarray_sum([1,-2,5,-2,6,-3]))